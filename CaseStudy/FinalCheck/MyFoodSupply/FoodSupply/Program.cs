...
namespace MyFoodSupply
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            FoodDetails medicine = new FoodDetails();
            ...
            try
            {
                ...
                FoodDetails chosenFoodItem = objMainProgram.CreateFoodDetail(foodItemName.Trim(), foodItemCategory, expiryDate, price);
                ...
                if (response.ToLower().Equals("y"))
                {
                    ...
                    Supply supplyDetail = objMainProgram.CreateSupplyDetail(count, requestDate, supplierName, packingCharge, chosenFoodItem);
                    ...
                    if (supplyDetail != null)
                    {
                        ...
                        Console.WriteLine("Total amount: " + supplyDetail.Cost.ToString());
                    }
                    Console.WriteLine();
        ...
        public FoodDetails CreateFoodDetail(string name, int dishType, DateTime expiryDate, double price)
        {
            ...
            FoodDetails foodItem = new FoodDetails()
            {
                Name = name,
                ExpiryDate = expiryDate,
                Price = price,
                DishType = (FoodDetails.Category)dishType
            };

            return foodItem;
        }

        public Supply CreateSupplyDetail(int foodItemCount, DateTime requestDate, string sellerName, double packingCharge, FoodDetails foodItem)
        {
            if (foodItem != null)
            {
                if (foodItemCount <= 0)
                ...
                Supply supplyDetail = new Supply()
                {
                    Count = foodItemCount,
                    RequestDate = requestDate,
                    SellerName = sellerName,
                    FoodItem = foodItem,
                    Cost = (foodItem.Price * foodItemCount) + packingCharge
                };
